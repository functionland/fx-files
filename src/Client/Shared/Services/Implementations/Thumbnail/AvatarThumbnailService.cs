...
public class AvatarThumbnailService : ThumbnailService, IAvatarThumbnailService
{
    ...
    {
        var uniqueKey = dId;
        return GetOrCreateThumbnailAsync("FulaAvatars", uniqueKey, async () => await IdentityService.GetAvatarAsync(dId), cancellationToken);
    }

    protected override IThumbnailPlugin? GetRelatedPlugin(string uri)
    {
        return ThumbnailPlugins.FirstOrDefault(plugin => plugin.IsExtensionSupported("jpg"));
    }
